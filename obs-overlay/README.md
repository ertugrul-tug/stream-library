...
- **🎲 !öner:** Kaptanın oyun kütüphanesinden (`steam-library.json`) rastgele bir oyun ve Steam linkini önerir; 30 sn'de bir. Sonraki 1 dakikada 🔥 yazan her kişi bir oy sayılır; oy alan oyunlar kumandada "Son yayınlar > Sohbetin istediği oyunlar" listesinde birikir (en fazla 10, ✕ ile silinir) ve "Yeni yayın" penceresindeki oyun listesinde 🔥 ile görünür.
- **🤫 Sessiz sohbet uyarısı:** Yayın 10 dakikayı geçmişken son 10 dakikada kimse yazmadıysa kumandanın üstünde sohbeti canlandırma hatırlatması çıkar; "Aktif sohbet (10 dk)" sayacı Seyir defterinde.
- **📅 !program:** Haftalık yayın saatini ve sıradaki seferi (24 saatten azsa kalan süreyle) söyler. Varsayılan Pazartesi–Cuma 20:30; değiştirmek için `show-config.json` içine `"weekly": {"days": [0,1,2,3,4], "time": "20:30"}` (0 = Pazartesi); `themes` ile her günün teması (sitedeki gibi Serbest, Hikayeli, Competitive, Survival, Toplu Ekip) mesaja eklenir. "Yayın Bitti" sahnesindeki veda mesajı ve ekrandaki özet kartı da sıradaki seferin gününü ve saatini gösterir.
- **⭐ Ekranda destek uyarısı:** Takip (toplu), abonelik/hediye abonelik, bit ve Kicks ekranda altın bir bantla teşekkür olarak çıkar; abonelikte ve 500+ bit/Kicks'te konfeti ve fanfar çalar. Başka bir uyarı servisi kullanılıyorsa Mini oyunlar'daki "⭐ Ekran uyarıları" düğmesiyle kapatılır. Önizleme: `overlay.html?sample=fx`.
- **🗓️ Haftanın mürettebatı:** Ganimet haftalık da sayılır; `!hafta` bu haftanın ilk 3'ünü söyler. Yeni haftanın ilk "Yeni yayın"ında geçen haftanın ilk 3'üne +100/+50/+25 ganimet verilir ve bot duyurur.
- **⏪ Açılış ekranı:** "Yayın Başlıyor" radar kartında geçen seferin özeti (oyun, mesaj, takip, skor, ganimet kralı) ve geçen haftanın ilk 3'ü görünür. Önizleme: `yayin-basliyor.html?sample`.
...
- **📱 Dikey klipler (Reels/Shorts/TikTok):** Her işaret yayın zamanının yanında OBS'in yerel kayıt zamanını da tutar. Yayından sonra, **"Yeni yayın"a basmadan önce** `python obs-overlay/make-clips.py` çalıştır: en yeni kayıttan her işaretin 30 sn öncesi + 10 sn sonrasını 1080×1920 keser (üstte kamera, altta oyun), birbirine yakın işaretleri tek klipte birleştirir, `kayıt klasörü/klipler/<kayıt adı>/` altına yazar. `--list` işaretleri gösterir, `--only 2,5` seçer, `--before`/`--after` süreyi değiştirir, `--preview` düzeni kontrol için tek kare PNG çıkarır. Kayıt klasörü (varsayılan `~/Videos`) ve kamera/oyun kırpma kutuları `show-config.local.json > clips` ile ayarlanır (`{"recordingDir": "...", "camera": [x, y, w, h], "game": [x, y, w, h]}`, karenin oranı olarak). ffmpeg gerekir: `winget install Gyan.FFmpeg`.
...
- **Mini oyunlar** (oyun sahnesinin üst ortasındaki etkinlik şeridinde görünür):
  ...
  - **🚂 Hype Train ve 📺 reklam:** Twitch Hype Train başlayınca etkinlik şeridinde mor "HYPE TRAIN" kartı açılır, her seviyede konfeti ve sohbet çağrısı olur, bitince teşekkür edilir. Reklam arası başlayınca bot sohbete süreyi söyler, kumandanın üstünde kalan süre sayılır (Streamer.bot `AdRun` / `HypeTrain*` olayları). Önizleme: `overlay.html?sample=hype`.
