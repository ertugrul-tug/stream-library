(() => {
  ...
  function playEffect(x) {
    ...
    } else if (x.type === 'support') {
      if (x.big) { confetti(); sfx.win(); } else arp([659, 880, 1047], 0.07, { type: 'triangle', gain: 0.05, dur: 0.22 });
      banner(x.name, (x.platform || '') + ' support');
    } else if (x.type === 'goal') {
      confetti();
  ...
  function render(s) {
    ...
    if (games) {
      const hy = s.hype, hc = games.hype;
      hc.classList.toggle('active', !!hy);
    ...
    if (crew) {
      ...
      const champs=(s.weekChamps&&s.weekChamps.top)||[];
      if (champs.length) div(crew,'show-small season-line week-line','🏆 GEÇEN HAFTANIN MÜRETTEBATI · '+champs.map((r,i)=>`${['🥇','🥈','🥉'][i]} ${r.name}`).join('  '));
      const last=(s.nights||[])[(s.nights||[]).length-1];
      if (last) {
        const bits=[last.game||'', last.chat?`💬 ${last.chat}`:'', last.follows?`👋 ${last.follows}`:'', last.wins+last.losses?`🏆 ${last.wins}-${last.losses}`:'', last.king?`👑 ${last.king}`:''].filter(Boolean);
        div(crew,'show-small last-line','⏪ GEÇEN SEFER · '+bits.join(' · '));
      }
    }
    if (log) {
    ...
    if (end) {
      ...
      if(s.nextShow) div(end,'show-small end-predict end-next',`📅 SIRADAKİ SEFER · ${String(s.nextShow).toLocaleUpperCase('tr-TR')}`);
    }
    if (spotlight) {
